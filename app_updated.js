...
let sales = JSON.parse(localStorage.getItem("touchPosSales")) || [];
...
function saveSales() {
  localStorage.setItem("touchPosSales", JSON.stringify(sales));
}
...
function renderSalesReport() {
  ...
  dailySales.forEach(sale => {
    paymentTotals[sale.paymentMethod] = (paymentTotals[sale.paymentMethod] || 0) + Number(sale.total || 0);
    grandTotal += Number(sale.total || 0);

    sale.items.forEach(item => {
      itemCount += Number(item.qty || 0);
      const key = item.id || item.name;
      const current = productTotals.get(key) || { name: item.name, qty: 0, sales: 0 };
  ...
  $("reportCash").textContent = money(paymentTotals.Cash);
  $("reportCard").textContent = money(paymentTotals.Card);
  $("reportTransfer").textContent = money(paymentTotals.Transfer);
  $("reportTotal").textContent = money(grandTotal);
  $("reportTransactions").textContent = dailySales.length;
  $("reportItems").textContent = itemCount;

  const rows = [...productTotals.values()].sort((a, b) => b.qty - a.qty || a.name.localeCompare(b.name));
...
$("clearReportBtn").addEventListener("click", () => {
  const selectedDate = $("reportDate").value || localDateKey();
  if (confirm(`Clear all saved sales for ${selectedDate}?`)) {
    sales = sales.filter(sale => sale.date !== selectedDate);
    saveSales();
    renderSalesReport();
  }
});
